...
| **Set clock (`#18`, ISO — DECOY)** | `#18 { #1 = "YYYY-MM-DDThh:mm:ss±hh:mm" }` | ISO-8601 local string; the app sends it on connect, but **HW-verified 2026-07-06 it is a NO-OP over BLE on both fw0107 and fw0111** (a +1 h set left the clock unchanged, no reply). **Use `#75` instead** to set the clock. |
| **Set clock (`#75 setEpochTime`) — THE setter** | `#75 { #1 = timeSecEpochUTC; #2 = timezoneOffsetSec }` | The real clock-set over BLE. `#1` = UTC epoch (uint32), `#2` = **signed** int32 tz offset (e.g. `-14400` for EDT → a 9–10-byte sign-extended varint). **HW-verified 2026-07-06 both families**: BTValve03 (fw0111) went from ~20 h drift to in-sync; the XD (fw0107) accepted a +1 h offset then restored. The app also uses `#75` to **trigger the first status burst** on connect. Matters for **programs**: start-times (`#8`) fire at *device-local* time, so a drifted clock waters at the wrong real-world hour — an off-app device (e.g. BTValve03) drifts until re-synced with `#75`. |
...
| Connect-time queries | `#15 {}`, `#22 {}`, `#45 {}`, `#120 {}` (empty), `#75 {#1=epochUTC; #2=tzOffsetSec}` (setEpochTime — the clock-set + first-burst trigger), `#18` ISO clock (decoy), `#19` reads of existing programs, device-info → RX `#23` | Sent during the handshake to set the clock + read current state. |
